...
const ChatContent = ({
      ...
      if (!isEnough) {
        try {
          ...
          const result = await dispatch(
            getMessageInCons({ id: params.id, page: lmao })
          ).unwrap();
          ...
          chatContentRef.current.scrollTop =
            chatContentRef.current.scrollHeight - height1;
        } catch (error) {
          throw error;
        }
      }
  ...
  const getMessagesInCons = async () => {
    try {
      ...
      const result = await dispatch(
        getMessageInCons({ id: params.id, page: 0 })
      ).unwrap();
  ...
  if (isOpenSetting) {
    ...
    return (
      <div className="rightPanel" style={{ position: "relative" }}>
                ...
                src={
                  conversations.find(
                    ...
                    ? conversations
                        ...
                        ?.members.find((item) => item._id !== currentUser._id)
                        .avatar
                    : conversations.find(
                        (conversation) => conversation._id === params.id
                      )?.members.length === 1
        ...
        {openImagePopup && (
          <ImagePopup src={srcPopup} setOpen={setOpenImagePopup} />
        )}
      </div>
    );
  }
